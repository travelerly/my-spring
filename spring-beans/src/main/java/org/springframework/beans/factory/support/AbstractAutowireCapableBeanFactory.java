...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		if (resolvedClass != null && !mbd.hasBeanClass() && mbd.getBeanClassName() != null) {
			/**
			 * 条件成立，则说明 beanClass 存的是 Class 的 name
			 * 则使用 mdb 深拷贝一个新的 RootBeanDefinition 副本，
			 * 并且将解析的 Class 赋值给拷贝的 RootBeanDefinition 副本的 beanClass 属性
			 */
		...
		try {
			// 验证及准备覆盖的方法（对 override 属性进行标记及验证）
		...
		try {
			/**
			 * 后置处理器可以介入进行增强，创建代理对象
			 * 创建对象之前留给我们一个机会，可以通过 BeanPostProcessors 创建并返回组件的"代理对象"，达到"短路"效果
			 * 实例化前的处理，给 InstantiationAwareBeanPostProcessor 一个机会返回代理对象来替代真正的 bean 实例，达到“短路”效果
			 * 例如在容器启动，创建第一个对象时，AnnotationAwareAspectJAutoProxyCreator 就会介入，
			 * 为 AOP 的所有切面生产所有的增强器并缓存起来
			 *
			 * 如果后置处理器介入，进行了增强，则会创建代理对象
			 * 如果后置处理器没有介入，则会返回 null
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// Bean 实例的包装类。Instantiate the bean.
		...
		if (instanceWrapper == null) {
			/**
			 * 1.实例化 bean 对象
			 * 创建 bean 的实例，且尚未属性赋值以及初始化，默认使用无参构造器创建对象
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					 ...
					 * 尤其是 bean 的属性上添加了 @Autoired、@Value、@Resource 等这些注解上面的信息是需要被纳入到 BeanDefinition 中
		...
		if (earlySingletonExposure) {
			...
			/**
			 * 暴露早期引用，将(未属性赋值、未初始化的半成品)对象 ObjectFactory 存入"单例工厂池"中，即三级缓存中，进行暴露，用于解决循环依赖
			 *
			 * 三级缓存：
			 ...
			 * singletonFactories：三级缓存，存放的是 ObjectFactory，是一个函数式接口，
			 * 当执行objectFactory#getObject() 方法时，最终会调用 getEarlyBeanReference(……)，来获取 bean 的早期引用
			 ...
			 * 2.三级缓存中存放的是 ObjectFactory 对象工厂，当执行 objectFactory#getObject() 时，会调用 getEarlyBeanReference(……)
		...
		if (earlySingletonExposure) {
			...
			if (earlySingletonReference != null) {
				...
				// 判断是否有其它 bean 依赖当前实例化的 bean
				else if (!this.allowRawInjectionDespiteWrapping && hasDependentBean(beanName)) {
					String[] dependentBeans = getDependentBeans(beanName);
					Set<String> actualDependentBeans = new LinkedHashSet<>(dependentBeans.length);
					for (String dependentBean : dependentBeans) {
						if (!removeSingletonIfCreatedForTypeCheckOnly(dependentBean)) {
							// 将那些依赖当组件的 bean 的名称添加到集合 actualDependentBeans 中
							actualDependentBeans.add(dependentBean);
						}
					}

					// 若依赖集合 actualDependentBeans 不为空，则抛出异常
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			// 判断当前 bean 是都是合成的 并且 当前容器是否存在实现了接口 InstantiationAwareBeanPostProcessors 的后置处理器
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				Class<?> targetType = determineTargetType(beanName, mbd);
				if (targetType != null) {
					/**
					 * bean 创建之前先进行增强（后置处理器执行前置处理方法）
					 * InstantiationAwareBeanPostProcessor 可以在此处尽心介入，可以在前置处理方法中创建代理对象
					 * 如果后置处理器对当前 beanName 创建出代理对象，则 Spring 会直接使用，不会再次创建
					 * 如果后置处理器没介入增强，则返回的 bean 为 null
					 */
					bean = applyBeanPostProcessorsBeforeInstantiation(targetType, beanName);
					if (bean != null) {
						// bean 创建完成后进行增强（后置处理器执行后置处理方法，创建了代理对象）
		...
		/**
		 * 后置处理器(InstantiationAwareBeanPostProcessor)进行了增强，则会返回创建的对象
		 * 后置处理器没有介入，则返回 null
		 */
		return bean;
	}
	...
	protected Object applyBeanPostProcessorsBeforeInstantiation(Class<?> beanClass, String beanName) {
		// 遍历所有的 "实例化后置处理器 InstantiationAwareBeanPostProcessor"
		for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
			/**
			 * 执行后置处理器的前置处理方法
			 * 如果在此后置处理器的前置处理方法中创建了对象，则 Spring 会直接使用这个对象，不会再次创建了
			 */
		...
		// 后置处理器没有介入或者后置处理器介入了，但没有创建对象，则返回 null
		return null;
	}
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		/**
		 * 如果当前 bean 不是被 public 修饰的，并且没有通过反射设置允许访问的权限，
		 * 此时就不能通过反射创建当前 bean 了，就会抛出异常
		 */
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		/**
		 * 准备所有的属性键值对。
		 * 从 BeanDefinition 中拿到所有属性的信息。指的是属性的键值对，属性名:属性值。
		...
		if (hasInstAwareBpps) {
			// 当前 Spring 容器中注册过 InstantiationAwareBeanPostProcessor 类型的后置处理器
		...
		if (needsDepCheck) {
			...
			// 属性依赖的检查，对应 depends-on 属性
			checkDependencies(beanName, mbd, filteredPds, pvs);
		}

		if (pvs != null) {
			/**
			 * 将收集到的属性信息，统一填充到 bean 实例中
	...
	protected void autowireByName(
		...
		for (String propertyName : propertyNames) {
			// 如果 Spring 容器中，已经存在属性依赖的 bean 实例
			if (containsBean(propertyName)) {
				// 通过属性的名称获取该属性的 bean 实例
				Object bean = getBean(propertyName);
				// 将属性的名称已经对应的依赖 bean 封装到 pvs 中
				pvs.add(propertyName, bean);
				// 注册属性和对应 bean 之间的依赖关系
	...
	protected void autowireByType(
		...
		// 用于存放，当 bean 的属性为集合类型时，需要依赖的 bean 的名称
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					...
					/**
					 * 对于集合类型的属性 autowiredBeanNames 中的每个元素都是当前 bean 依赖的
					 * autowiredBeanNames 在按照类型装配过程中存在的意义：
					 * 1.在匹配的过程中，可能会同时匹配到多个类型相同的 bean
					 * 2.若当前 bean 的属性类型是包含的集合类型，例如：List<Student>、Set<Student>等，则需要根据类型匹配到多个 bean，
					 *   就需要将这些 bean 的信息都存放到集合 autowiredBeanNames 中，方便后续和 bean 绑定依赖关系
					 */
	...
	protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
		...
		for (PropertyDescriptor pd : pds) {
			// 属性存在对应的 setter 方法 && 依赖检查属性的类型没有被添加到忽略的感知接口中 && pvs 中不存在该属性名称 && 不是简单的数据类型（即为引用类型）
	...
	protected boolean isExcludedFromDependencyCheck(PropertyDescriptor pd) {
		// AutowireUtils.isSetterDefinedInInterface()：判断属性的类型是否被添加到忽略的感知接口中
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		if (pvs instanceof MutablePropertyValues) {
			...
			if (mpvs.isConverted()) {
				...
				try {
					// 如果 mpvs 中的值已经被转换为对应的类型，那么可以将其直接设置到 BeanWarper 中
		...
		// 若调用者没有自定义转换器，那么就是用 BeanWrapper 本身，因为 BeanWrapper 实现了 TypeConverter 接口
		TypeConverter converter = getCustomTypeConverter();
		if (converter == null) {
			converter = bw;
		}

		// 创建属性解析器，主要完成属性值的处理，包括依赖其他 bean 的创建
		...
		// 遍历属性，将属性转换为对应的类的对应属性的类型
		for (PropertyValue pv : original) {
			if (pv.isConverted()) {
				// 如果 pv 中包含了转换的值，则直接添加到深拷贝集合中
				deepCopy.add(pv);
			}
			else {
				// 否则进行属性转换
				...
				// 使用解析器解析不同类型的值（包括循环依赖的解决）
				Object resolvedValue = valueResolver.resolveValueIfNecessary(pv, originalValue);
				Object convertedValue = resolvedValue;
				boolean convertible = bw.isWritableProperty(propertyName) &&
						!PropertyAccessorUtils.isNestedOrIndexedProperty(propertyName);

				// 属性类型装换器，如果有定义的类型转换，就发生在 convertForProperty() 中
				if (convertible) {
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		...
		if (System.getSecurityManager() != null) {
			...
			/**
			 * 调用实现了 Aware 接口(感知接口)中的方法
			 * 例如当前 bean 实现了 BeanNameAware、BeanClassLoaderAware 或 BeanFactoryAware 接口
			 */
			invokeAwareMethods(beanName, bean);
		}
		...
		if (mbd == null || !mbd.isSynthetic()) {
			/**
			 * 执行后置处理器的 beforeInitialization 方法(初始化之前执行)
			 */
		...
		if (mbd == null || !mbd.isSynthetic()) {
			 ...
			 * 2.存在循环依赖的情况下，AOP 的代理对象在属性赋值的时候，在依赖对象的初始化过程中创建完成了，并设置了代理对象已经创建的表示
			 *   此处判断后会直接跳过，不再重复创建代理对象
			 */
	...
	protected void invokeInitMethods(String beanName, Object bean, @Nullable RootBeanDefinition mbd)
		...
		// 如果组件实现了生命周期 InitializingBean 接口，就调用组件自己的 afterPropertiesSet 方法，bean 属性设置完之后执行
