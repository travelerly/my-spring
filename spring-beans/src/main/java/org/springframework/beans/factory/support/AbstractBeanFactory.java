...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	/** 保存所有的 bean 后置处理器 BeanPostProcessors to apply. */
	...
	protected <T> T doGetBean(
		 ...
		 * 因为该 beanName 对象的 bean 有可以能已经被创建过，因此需要先从缓存中获取
		...
		/**
		 * 如果是正常获取 bean，是通过 getBean(beanName) 方法调用至此，args 为 null
		 * 如果调用至此，args 不为 null，意味着调用放不是希望获取 bean，而是希望创建 bean
		 */
		if (sharedInstance != null && args == null) {
			...
			/**
			 * 获取 beanName 对应的实例对象
			 * 主要用于 FactoryBean 的特殊处理，普通 bean 会返回 sharedInstance 本身，即返回缓存中的 bean 对象
			 */
			...
			if (isPrototypeCurrentlyInCreation(beanName)) {
				 ...
				 * scope 为 prototype 的循环依赖校验：
				 * 如果 beanName 已经正在创建 Bean 实例中，而此时我们又要再一次创建 beanName 的实例，则代表出现了循环依赖，需要抛出异常。
				 * 例如：如果存在 A 中有 B 的属性，B 中有 A 的属性，
				 * 		那么当依赖注入的时候，就会产生当 A 还未创建完的时候因为对于 B 的创建再次返回创建 A，造成循环依赖
				 */
				throw new BeanCurrentlyInCreationException(beanName);
			}

			// 拿到当前容器的的父容器。 if bean definition exists in this factory.
			...
			try {
				...
				// 根据 beanName 获取 BeanDefinition 定义信息，并封装成 RootBeanDefinition
				RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
				// 检查一下当前 RootBeanDefinition 是否为抽象类，如果是则会抛出异常
				checkMergedBeanDefinition(mbd, beanName, args);

				/**
				 * 获取当前 bean 依赖(@DependsOn 中依赖)的所有 bean 的名称集合，
				 * 在实例化当前 bean 之前，需要先将其所依赖的 bean 完成实例化
				...
				if (dependsOn != null) {
					for (String dep : dependsOn) {
						...
						try {
							// 递归调用 getBean() 方法，提前实例化当前 bean 所依赖的那些 bean（依赖了其他 bean，要先获取其他 bean）
							getBean(dep);
						}
				...
				if (mbd.isSingleton()) {
					...
					sharedInstance = getSingleton(beanName, () -> {
						try {
							/**
							 * 创建 bean 实例，开启完整的 bean 生命周期干预流程
				...
				/**
				 * 创建"多实例" bean 对象
				 * 当前 beanDefinition 为原型 prototype 类型的多实例，
				 */
				...
				/**
				 * 其他作用域范围，
				 * 例如 @RefreshScope 标注的类，在发生 RefreshScopeRefreshedEvent 事件时，spring 缓存中的 bean 被清空，
				 * 当再次使用这个 bean 的时候，需要重新构建 bean，因此会调用至此处创建 bean 实例
				 */
				else {
					...
					if (scope == null) {
						// 如果没有为 BeanDefinition 设置 scope，就抛出异常
	...
	protected boolean isTypeMatch(String name, ResolvableType typeToMatch, boolean allowFactoryBeanInit)
		...
		// 先从缓存中查询当前组件名称对应的实例。Check manually registered singletons.
		Object beanInstance = getSingleton(beanName, false);
		if (beanInstance != null && beanInstance.getClass() != NullBean.class) {
			if (beanInstance instanceof FactoryBean) {
				if (!isFactoryDereference) {
					...
					// 判断工厂方法返回的类型(type)，与指定的类型(typeToMatch)进行对比，判断类型是否一致
	...
	public Class<?> getType(String name, boolean allowFactoryBeanInit) throws NoSuchBeanDefinitionException {
		...
		// 根据 beanName，到单例缓存池中查找其对相应的实例对象。Check manually registered singletons.
		Object beanInstance = getSingleton(beanName, false);
		if (beanInstance != null && beanInstance.getClass() != NullBean.class) {
		...
		if (beanClass != null && FactoryBean.class.isAssignableFrom(beanClass)) {
			...
			// 返回 Class 对象，例如："class com.coLin. bean.cycte.B"
	...
	BeanPostProcessorCache getBeanPostProcessorCache() {
		...
		if (bpCache == null) {
			...
			// 遍历所有 bean 后置处理器，筛选出 "实例化后置处理器 InstantiationAwareBeanPostProcessor"，并进行缓存
			for (BeanPostProcessor bp : this.beanPostProcessors) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					// 将 "实例化后置处理器 InstantiationAwareBeanPostProcessor" 缓存起来
					bpCache.instantiationAware.add((InstantiationAwareBeanPostProcessor) bp);
					if (bp instanceof SmartInstantiationAwareBeanPostProcessor) {
						// 缓存 SmartInstantiationAwareBeanPostProcessor
	...
	protected Object getObjectForBeanInstance(
		...
		if (BeanFactoryUtils.isFactoryDereference(name)) {
			...
			// 如果 name 是以 "&" 为前缀，相应的 beanInstance 必须是 FactoryBean 的实例，否则抛出异常
			if (!(beanInstance instanceof FactoryBean)) {
		...
		if (!(beanInstance instanceof FactoryBean)) {
			// 如果 name 不是以 "&" 为前缀，并且 beanInstance 也不是 FactoryBean 的实例，则直接返回该单例 bean
			return beanInstance;
		}

		/**
		 * 以下是对 name 不是以 "&" 为前缀，且 beanInstance 为 FactoryBean 实例的情况的处理
		 * 说明 beanInstance 为 FactoryBean 的实例，会通过 FactoryBean 来实例化这个 bean
		 */
		...
		if (mbd != null) {
			...
			// 从缓存中获取 FactoryBean 实例化好的 bean
			object = getCachedObjectForFactoryBean(beanName);
		}
		if (object == null) {
			// 类型装换。Return bean instance from factory.
			FactoryBean<?> factory = (FactoryBean<?>) beanInstance;
			// Caches object obtained from FactoryBean if it is a singleton.
			if (mbd == null && containsBeanDefinition(beanName)) {
				// 获取 BeanDefinition，并封装为 RootBeanDefinition
				mbd = getMergedLocalBeanDefinition(beanName);
			}
			boolean synthetic = (mbd != null && mbd.isSynthetic());
			// 通过 FactoryBean 来实例化这个 bean
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		...
		if (!mbd.isPrototype() && requiresDestruction(bean, mbd)) {
			if (mbd.isSingleton()) {
				...
				// 为 bean 注册 DisposableBean 的实现类，也就是 bean 销毁相关的方法
