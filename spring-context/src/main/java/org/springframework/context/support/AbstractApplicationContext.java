...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	/** 工厂后置处理器集合。BeanFactoryPostProcessors to apply on refresh. */
	...
	/** 策略模式的环境类。ResourcePatternResolver used by this context. */
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			/**
			 * 一、准备刷新，刷新预处理，和主流程关系不大，就是保存了容器的启动时间、启动标志等，初始化上下文环境
			 * 1.设置容器的启动时间
			 * 2.设置活跃状态为 true
			 * 3.设置关闭状态为 true
			...
			/**
			 * 三、BeanFactory 的前置操作，即对 BeanFactory 进行属性填充，
			 ...
			 * 还设置了忽略自动转配和允许自动转配的接口，如果不存在某个 bean 的时候，Spring 就自动注册单例 bean
			 * 还设置了 bean 表达式解析器
			 */
			...
			try {
				/**
				 * 四、留给子类的模板方法，允许子类继续对工厂执行一些处理(注册一些特殊的后置处理器)
				...
				/**
				 * 五、工厂的增强：调用执行所有的 BeanFactory 的后置处理器，对工厂进行修改或增强(配置类会在这里解析)。
				 * 1.会在此将 class 扫描成 BeanDefinition
				 * 2.调用 bean 工厂的后置处理器
				...
				/**
				 * 六、注册所有的 bean 的后置处理器 BeanPostProcessor，但不会调用
				 * 例如：
				 ...
				 * CommonAnnotationBeanPostProcessor：处理被 @PreDestroy、@PostConstruct、@Resource 等多个注解修饰的方法
				 */
				registerBeanPostProcessors(beanFactory);
				beanPostProcess.end();

				/**
				 * 七、初始化消息源，即初始化国际化资源处理器 MessageResource。
				 * 观察容器中是否含有 MessageResource 的定义信息，如果没有就注册一个并放到单例池中
				 * Initialize message source for this context.
				 */
				initMessageSource();

				/**
				 * 八、初始化事件多播器，之后注册的监听器和发布事件都是基于该事件多播器执行的
				...
				/**
				 * 十、初始化(注册)各种监听器，关联 Spring 的事件监听机制。
				...
				/**
				 * 十二、初始化生命周期处理器，并刷新，找出 Spring 容器中实现了 Lifecycle 接口的 bean，并执行 start() 方法
	...
	protected void prepareRefresh() {
		...
		/**
		 * 其他子容器自行实现，
		 * 例如：web-ioc容器在启动时，一般在此加载当前应用的上下文信息，ApplicationContext
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		/**
		 * 设置 bean 工厂的类加载器为当前 Application 应用的类加载器
		 */
		beanFactory.setBeanClassLoader(getClassLoader());
		if (!shouldIgnoreSpel) {
			// 为 beanFactory 设置 #{……} SPEL 表达式的解析器
			beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		}

		// 为 bean 工厂设置一个 propertityEditor 属性资源编辑器，用于给后面的 bean 对象赋值使用
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		/**
		 * 为 beanFactory 注册 Aware 接口(感知接口)方法的后置处理器。
		 * 注册一个完整的 ApplicationContextAwareProcessor 后置处理器，用来处理 ApplicationContextAware 接口的回调方法
		 ...
		 * 因为以下接口都有 setXXX() 方法，若这些方法不做特殊处理，都会自动注入容器中的 bean
		 */
		...
		/**
		 * 以下接口，允许自动装配，第一个参数是自动装配的类型，第二个参数是自动装配的值
		 * 为 beanFactory 注册指定接口的依赖实现类
		 * 当注册了依赖解析后，例如当注册了对 BeanFactory.class 的解析依赖后，
		 * 当 bean 的属性注入的时候，一旦检测到属性为 BeanFactory 类型，便会将 beanFactory 的实例注册进去。
		 *
		 * @Autowired
		 * ApplicationContext applicationContext，
		 * 如能够通过注解 @Autowired 注入属性 applicationContext，就是因为这里设置了
		 */
		 ...
		 * 添加一个后置处理器 ApplicationListenerDetector，其实现了 BeanPostProcessor 接口
		 */
		...
		// 注册上下文环境相关的默认组件 bean。Register default environment beans.
	...
	protected void initMessageSource() {
		// 获取容器，即获取 bean 工厂，DefaultListBeanFactory
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 判断容器中是否存在名称为 messageSource 的 bean
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			// 如果存在这个 bean，则从 bean 工厂中获取这个 bean 对象
			this.messageSource = beanFactory.getBean(MESSAGE_SOURCE_BEAN_NAME, MessageSource.class);
			// Make MessageSource aware of parent MessageSource.
			if (this.parent != null && this.messageSource instanceof HierarchicalMessageSource) {
				// 当父类 bean 工厂不为空，并且这个 messageSource 的 bean 对象是 HierarchicalMessageSource 类型的，则转换其类型
				HierarchicalMessageSource hms = (HierarchicalMessageSource) this.messageSource;
				if (hms.getParentMessageSource() == null) {
					// Only set parent context as parent MessageSource if no parent MessageSource
					// registered already.
					// 设置父类 messageSource，此处设置内部的 parent messageSource
			...
			/**
			 * 初始化 DelegatingMessageSource 类型的消息源对象
			 * 如果没有 xml 文件定义细腻源对象，则新建 DelegatingMessageSource 类型的 messageSource 的 bean
			 * 因为 DelegatingMessageSource 实现了 HierarchicalMessageSource 接口，为这个接口继承了 MessageSource，
			 * 因此实现了接口 HierarchicalMessageSource 的类，都是 MessageSource 的子类
	...
	protected void initApplicationEventMulticaster() {
		// 获取容器
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 判断容器中是否有名称为 applicationEventMulticaster 的 bean
	...
	protected void initLifecycleProcessor() {
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 判断 Spring 容器中，是否存在名称为 lifecycleProcessor 的 bean
		if (beanFactory.containsLocalBean(LIFECYCLE_PROCESSOR_BEAN_NAME)) {
			...
			// 如果容器中不存在 lifecycleProcessor，则创建一个 DefaultLifecycleProcessor 类型的 bean
			DefaultLifecycleProcessor defaultProcessor = new DefaultLifecycleProcessor();
			defaultProcessor.setBeanFactory(beanFactory);
			// 设置成员变量 lifecycleProcessor
	...
	protected void registerListeners() {
		...
		/**
		 * 获取之前保存的早期事件，派发这些早期事件(earlyApplicationEvents)。
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果 BeanFactory 之前没有注册嵌入值解析器，则注册一个嵌入值解析器，主要用于注解属性值的解析
		...
		/**
		 * 从工厂中获取所有实现了 LoadTimeWeaverAware 接口的实现类的名称
		 * LoadTimeWeaverAware：aspectj :加载时织入功能「aop」
		 * Initialize LoadTimeWeaverAware beans early to allow for registering their transformers early.
		 */
		String[] weaverAwareNames = beanFactory.getBeanNamesForType(LoadTimeWeaverAware.class, false, false);
		for (String weaverAwareName : weaverAwareNames) {
			// 提前调用 getBean()，尽早实例化这些名称对应的 bean
		...
		// 冻结所有的 bean 定义，注册的 bean 定义不会被修改或进一步后置处理，因为即将要创建 bean 实例对象了
