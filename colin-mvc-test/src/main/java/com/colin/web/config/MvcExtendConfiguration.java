 ...
 *
 * SpringMVC 的两种启用方式
 * 1.使用注解 @EnableWebMvc 开启 SpringMVC 功能，此方式修改了 SpringMVC 底层行为，
 *   只需要实现 WebMvcConfigurer 接口，重写方法来配置组件功能。
 *   WebMvcConfigurer 预留了 SpringMVC 的扩展接口，可扩展 SpringMVC 的很多功能组件；
 *
 * 2.SpringMVC 默认规则，即所有组件都是在 DispatcherServlet 初始化的时候直接使用配置文件中指定的默认组件。
 ...
 *    1. 任意配置类上加注解 @EnableWebMvc，然后实现 WebMvcConfigurer 接口，进行扩展；
